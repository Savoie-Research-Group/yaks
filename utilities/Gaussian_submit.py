...
def main(argv):
    ...
    if len(args.file_in_folder) == 0:
        Files[Filename] = [ os.path.join(dp, f) for dp, dn, filenames in os.walk(args.path) for f in filenames if fnmatch.fnmatch(f,Filename) ]
    else:
