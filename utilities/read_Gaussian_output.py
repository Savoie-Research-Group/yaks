...
def read_geoopt(inputF,output,Element_num,count_flag=False,scale=0.3):
    ...
    with open(inputF,'r') as f:
        for lc,lines in enumerate(f):
            ...
            if 'Depolar' in fields and '(P)' in fields: blank = 8

            if len(fields) == 2 and fields[0] == "Standard" and fields[1] == "orientation:":
            #if len(fields) == 2 and fields[0] == "Input" and fields[1] == "orientation:":
                count_xyz += [lc]
    ...
    if imag_flag == False:
        ...
        with open(inputF,'r') as f:
            for lc,lines in enumerate(f):
                if lc >= imag_lc+blank and lc < imag_lc+blank+Element_num:
                    fields = lines.split()
                    if image_number == 1:
...
def read_IRC(inputF,output,Element_num):
    ...
    with open(inputF,'r') as f:
        for lc,lines in enumerate(f):
            ...
            if len(fields) >= 6 and fields[0] == 'Redundant' and fields[1] == 'internal' and fields[2] == 'coordinates':
                E_count = lc
    ...
    with open(output,'w') as f:
        for i in range(N_image+1)[1:]:
            f.write('{}\n'.format(Element_num))
            f.write('Image {} energy:{} \n'.format(i,geo_dict[str(i)][0]))
