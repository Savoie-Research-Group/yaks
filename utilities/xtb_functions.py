...
################################################################################################################################################
#
############################################### Function to write xcontrol file for geo_opt/CREST ##############################################
# 
################################################################################################################################################
def write_xcontrol(cons_file,reference,fixed_atoms,relax_atoms,force_const=1.0):
    
    # set fixed/relaxed atom strings
    fixed_atom_list,relax_atom_list=[],[]
    for ind in fixed_atoms: 
        if len(ind) == 2:
            fixed_atom_list += ["{}-{}".format(ind[0]+1,ind[1]+1)]
        elif len(ind) == 1:
            fixed_atom_list += ["{}".format(ind[0]+1)]
        else:
            print("Please specify correct fix atom list")

    for ind in relax_atoms: 
        if len(ind) == 2:
            relax_atom_list += ["{}-{}".format(ind[0]+1,ind[1]+1)]
        elif len(ind) == 1:
            relax_atom_list += ["{}".format(ind[0]+1)]
        else:
            print("Please specify correct relax atom list")

    fixed_atom_string = ','.join(fixed_atom_list)
    relax_atom_string = ','.join(relax_atom_list)

    with open(cons_file,'w') as f:
        f.write("$constrain\n")
        f.write("    force constant={}\n".format(force_const))
        f.write("    atoms: {}\n".format(fixed_atom_string))
        f.write("    reference={}\n".format(reference))
        f.write("$metadyn\n")
        f.write("    atoms: {}\n".format(relax_atom_string))
        f.write("$end")
