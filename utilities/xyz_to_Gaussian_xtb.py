...
def main(argv):
    ...
    parser.add_argument('-c', dest='chk_name', default=None,
                        help = 'name of check point file, if is none, no chk file will be assigned')

    parser.add_argument('-cs', dest='check_state', default=1,
                        help = 'check point state, if is 1, chk= will be written into gaussian input file, otherwise oldchk= will be written. (Only valid if chk_name is not None)')

    parser.add_argument('-eps', dest='dielec_const', default=0.0,
                        help = 'Dielectric constant if you need. (default=0.0)')
    ...
    with open(args.output_name,'w') as f:
        if args.chk_name:
            if int(args.check_state) == 1:
                f.write("%chk={}.chk\n".format(args.chk_name))
            else:
                f.write("%oldchk={}.chk\n".format(args.chk_name))
