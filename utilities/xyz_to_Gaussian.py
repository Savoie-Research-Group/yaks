...
def main(argv):
    ...
    parser.add_argument('-T', dest='temperature', default=None,
                        help = 'Sets the temperature, deault None (298.15K)')

    parser.add_argument('-S', dest='solvent', default=None,
                        help = 'Sets the Gaussian Solvent model, asks for A/B where A refers to model and B refers to solvent, default: None ')
    ...
    parser.add_argument('-eps', dest='dielec_const', default=0.0,
                        help='Dielectric constant for your system if you need. (default: 0.0)')
    ...
    args.dielec_const=float(args.dielec_const)
    # append temperature
    if args.temperature is not None:
        args.ty += ' Temperature={}'.format(args.temperature)

    if args.solvent is not None:
        if '/' not in args.solvent: 
            print("expect A/B format where A refers to model and B refers to solvent, treat as None...")
        else:
            solvent = args.solvent
            args.ty += ' SCRF=({},solvent={})'.format(solvent.split('/')[0],solvent.split('/')[1])
    ...
    with open(args.output_name,'w') as f:
        ...
        for count_i,i in enumerate(Geometry):
            ...
            f.write("\n")
        f.write("\n")
        if args.dielec_const != 0.0:
            f.write('solventname=newsolvent\n')
            f.write('eps={}\n'.format(args.dielec_const))
            f.write('\n')
