...
def return_smi(E,G, adj_mat=None,q_tot=0,namespace='obabel'):
    if adj_mat is None and q_tot==0:
        xyz_write("{}_input.xyz".format(namespace),E,G, q_tot=q_tot)
        ...
        E=[_.upper() for _ in E]
        mol_write("{}_input.mol".format(namespace),E,G,adj_mat, q=q_tot)
        #from rdkit import Chem
        #mol=Chem.MolFromMolFile("{}_input.mol".format(namespace), sanitize=False)
...
# Function to Return smiles string 
def return_inchikey(E,G,adj_mat=None, q_tot=0, separate=False,namespace='obabel'):
    ...
    if separate:
        ...
        for group in groups:
            ...
            # generate inchikeu
            mol_write("{}_input.mol".format(namespace),frag_E,frag_G,frag_adj, q=q_tot)
        ...
        for i in range(len(smiles)):
            if smiles[i] == '+':
                if i+1 < len(smiles) and smiles[i+1].isdigit():
                    positive_charge += int(smiles[i+1])
                else:
                    positive_charge += 1
            elif smiles[i] == '-':
                if i+1 < len(smiles) and smiles[i+1].isdigit():
                    negative_charge += int(smiles[i+1])
                else:
                    negative_charge += 1
        q=positive_charge-negative_charge
        m2= Chem.AddHs(m)
    ...
    with open(input_xyz,"r") as f:
        for lc,lines in enumerate(f):
            ...
            if '/' in lines:
                xyz[count]+="\n"
                continue
            elif len(fields) == 1 and float(fields[0]) == float(N):
                count+=1
...
def read_Gaussian_output(file_name):
    ...
    # if the imaginary frequencies is less than 50 cm**-1, turn imag_flag to False
    if imag_flag:
        imag_line = out_lines[imag_line+9]
        if abs(float(imag_line.split()[2])) < 50:
...
def read_pyGSM_output(output_folder,TS_folder,N_dict):
    ...
    if os.path.exists(output_folder+'/log') is False: return False
...
def return_Rindex(E,RG,PG=None,Radj_mat=None,Padj_mat=None):
    ...
    for dup in duplicates:
        ...
        if sorted(Rgroup) != sorted(Pgroup):
            non_dup += [dup]

    # remove "non"-dup compound from dup
    for dup in non_dup: duplicates.remove(dup)
...
def channel_classification(output_path,initial_xyz,inp_list,low_IRC_dict,N_dict,adj_dict,F_dict,reaction_dict,TS_folder=None,append=True,model='IRC_model.json',thresh=1.0,package='Gaussian',dg_thresh=None,IRC_xyz=None):
    ...
    for TSinp in inp_list:
        ...
        if imag_flag and finish_flag:
            ...
            # predict intended score
            I_score  = predict_TS_intended(E,RG,PG,'{}/{}-TS.out'.format(TS_folder,pname),model=model,package=package,Radj_mat=Radj_mat,Padj_mat=Padj_mat,lowTS_xyz='{}/{}/{}-TS.xyz'.format(output_path,pname,pname),\
                                           highTS_xyz='{}/{}-TS.xyz'.format(TS_folder,pname),low_IRC=low_IRC_dict[pname])
            if I_score > thresh: TStype = 'Intended'

            # predict the intended score of xTB-IRC direction
            if IRC_xyz:
                IRC_xyz_file    = '{}/{}-IRC.xyz'.format(IRC_xyz,pname)
                _,RG_low,PG_low,Radj_mat_low,Padj_mat_low = parse_IRC_xyz(IRC_xyz_file,len(E))

                I_score_low     = predict_TS_intended(E,RG_low,PG_low,'{}/{}-TS.out'.format(TS_folder,pname),model=model,package=package,Radj_mat=Radj_mat_low,Padj_mat=Padj_mat_low,\
                                                      lowTS_xyz='{}/{}/{}-TS.xyz'.format(output_path,pname,pname),highTS_xyz='{}/{}-TS.xyz'.format(TS_folder,pname),low_IRC=low_IRC_dict[pname])
                
                # if xTB-IRC direction has larger score. replace the intended direction by xTB IRC direction, and this will be an unintended reaction
                if I_score_low > I_score: 
                    RG,PG    = RG_low,PG_low
                    Radj_mat, Padj_mat = Radj_mat_low, Padj_mat_low
                    I_score  = I_score_low
                    TStype   = 'Unintended'
                    _,_,Rhash_list_xTB=canon_geo(E,Radj_mat)
                    _,_,Phash_list_xTB=canon_geo(E,Padj_mat)
                    
            # get the smiles string of intended product
            Rsmiles  = return_smi(E,RG,Radj_mat)
            ...
            if I_score < thresh:
                ...
                if TStype  == 'Intended':
                    ...
                    with open(output_path+'/report.txt','a') as g:
                        g.write('{:<40s} {:<60s} {:<15s} {:<15.4f}\n'.format(pname,Psmiles,TStype,(F_298-F_dict[pname])*627.5))
                else:
                    
                    if np.abs(np.array(Rhash_list_xTB)-np.array(Rhash_list)).sum() == 0: 
                        print("TS for reaction payway to {} is found with Energy barrier {}".format(Psmiles,(F_298-F_dict[pname])*627.5))
                        # write intended channel into the report
                        with open(output_path+'/report.txt','a') as g:
                            g.write('{:<40s} {:<60s} {:<15s} {:<15.4f}\n'.format(pname,Psmiles,TStype,(F_298-F_dict[pname])*627.5))
                    ...
                    elif np.abs(np.array(Phash_list_xTB)-np.array(Rhash_list)).sum() == 0:
                        print("TS for reaction payway to {} is found with Energy barrier {}".format(Rsmiles,(F_298-F_dict[pname])*627.5))
                        # write intended channel into the report
                        with open(output_path+'/report.txt','a') as g:
                            g.write('{:<40s} {:<60s} {:<15s} {:<15.4f}\n'.format(pname,Rsmiles,TStype,(F_298-F_dict[pname])*627.5))

                    else: 
                        print("Unintended reaction found for chnnel {}, both two nodes do not match reactant...".format(pname))

                # write into reaction database if is needed
                if append:
                    G1,G2,adj_1,adj_2,rtype=RG,PG,Radj_mat,Padj_mat,TStype
...
def read_IRC_outputs(output_path,IRCinp_list,adj_dict,hash_dict,N_dict,reaction_dict,F_dict=None,TSEne_dict=None,append=False,select=False,dg_thresh=None,folder_name='IRC-result',q_tot=0,package='Gaussian',criterion='S'):
    # make folder for DFT 
    ...
    for IRCinp in IRCinp_list:
        ...
        smiles1 = return_smi(NE,NG1,N_adj_1, q_tot=q_tot)
        ...
        smiles2 = return_smi(NE,NG2,N_adj_2, q_tot=q_tot)
...
def check_DFT_energy(reactant_smiles,E_dict,conf_path,functional,basis,config,dispersion=None,solvation=None,eps=0.0,temperature=None,package='Gaussian'):
    ...
    for smi in reactant_smiles:
        ...
        if inchi_s not in E_dict.keys():
            ...
            bond_matrix, _=find_lewis(E, adj_mat, q=q)
            lone_electrons, fc, bonding_electrons, core_electrons=count_bond_matrix(E, bond_matrix)
            ...
            if os.path.isdir(conf_path+'/{}'.format(inchi_s)) is False and os.path.isdir(conf_path+'/{}'.format(inchikey)) is False:
                ...
                if os.path.exists(conf_path+'/{}/crest_best.xyz'.format(inchi_s)): 
                    geo_file = conf_path+'/{}/crest_best.xyz'.format(inchi_s)
                else:
                    geo_file = conf_path+'/{}/results/0.xyz'.format(inchi_s)
            else:
                if os.path.exists(conf_path+'/{}/crest_best.xyz'.format(inchikey)):
                    geo_file = conf_path+'/{}/crest_best.xyz'.format(inchikey)
                else:
                    geo_file = conf_path+'/{}/results/0.xyz'.format(inchikey)
            ...
            if package.lower() == 'gaussian':
                ...
                if solvation is not None and eps==0.0: level += ' SCRF=({},solvent={})'.format(solvation.split('/')[0],solvation.split('/')[1])
                if solvation is not None and eps!=0.0: level += ' SCRF=(Read)'
                if len(E) == 1: # we don't need to use opt option
                    substring = "python {}/utilities/xyz_to_Gaussian.py {} -o DFT/{}.gjf -eps {} -q {} -m {} -c \"{}\" -ty \"{} Opt=(maxcycles=100) Int=UltraFine SCF=QC Freq\" -t \"{} DFT\" -T {} "
                else: substring = "python {}/utilities/xyz_to_Gaussian.py {} -o DFT/{}.gjf -eps {} -q {} -m {} -c \"{}\" -ty \"{} Int=UltraFine SCF=QC Freq\" -t \"{} DFT\" -T {}"
                substring = substring.format('/'.join(os.getcwd().split('/')[:-1]),geo_file,inchikey,eps,q,multiplicity,"False",level,inchikey,str(temperature))
                ...
                option=""
                if len(E)==1 and float(eps)==0.0: option="Freq"
                elif float(eps)==0.0: option="Opt Freq"
                elif len(E)==1 and float(eps)!=0.0: option="Freq cpcm"
                else: option="Opt Freq cpcm"
                if temperature is not None:
                    if float(eps)==0.0: substring = "python {}/utilities/xyz_to_Orca.py {} -p {} -o DFT/{} -q {} -m {} -T {} {} -eps {} -ty \"{}\"  "
                    else: substring = "python {}/utilities/xyz_to_Orca.py {} -p {} -o DFT/{} -q {} -m {} -T {} {} -eps {} -ty \"{}\"  "
                    substring = substring.format('/'.join(os.getcwd().split('/')[:-1]),geo_file,config["procs"],inchikey,q,multiplicity,temperature,level,eps, option)
                else:
                    if float(eps)==0.0: substring = "python {}/utilities/xyz_to_Orca.py {} -p {} -o DFT/{} -q {} -m {} {} -eps {} -ty \"{}\"  "
                    else: substring = "python {}/utilities/xyz_to_Orca.py {} -p {} -o DFT/{} -q {} -m {} {} -eps {} -ty \"{}\"  "  
                    substring = substring.format('/'.join(os.getcwd().split('/')[:-1]),geo_file,config["procs"],inchikey,q,multiplicity,level,eps,option)
                output = subprocess.Popen(substring,shell=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE).communicate()[0]            
    ...
    if len(R_list) > 0:
        ...
        print("Missing list is: {}".format(R_list))
        return False,R_list
...
def gen_external_orca(nprocs, output='./', level='B97-3c', eps=0.0):
    # This function is generate the shell file for running the functional in ORCA by Gaussian
    out=open(output+'/external.sh', 'w+')
    out.write('#!/bin/bash\n')
    out.write('# Set the number of cores and maximum memory\n')
    out.write('module unload openmpi\n')
    out.write('export PATH=/depot/bsavoie/apps/orca_5_0_1_openmpi411:$PATH\n')
    out.write('export LD_LIBRARY_PATH=/depot/bsavoie/apps/orca_5_0_1_openmpi411:$LD_LIBRARY_PATH\n')
    out.write('export PATH=/depot/bsavoie/apps/openmpi_4_1_1/bin:$PATH\n')
    out.write('export LD_LIBRARY_PATH=/depot/bsavoie/apps/openmpi_4_1_1/lib:$LD_LIBRARY_PATH\n')
    out.write('read atoms derivs charge spin < $2\n')
    out.write('if [ $derivs == \"2\" ] ; then\n')
    out.write('    task=\"engrad freq\"\n')
    out.write('elif [ $derivs == \"1\" ] ; then\n')
    out.write('    task=\"engrad\"\n')
    out.write('fi\n')
    out.write('echo \"Generating mol.in\"\n')
    if eps==0.0:
        out.write('cat > mol.in <<EOF\n')
        out.write('! {} tightscf $task BOHRS nopop\n'.format(level))
        out.write('%pal nprocs {} end\n'.format(nprocs))
        out.write('%maxcore 8000\n')
        out.write('* xyz $charge $spin\n')
        out.write('$(sed -n 2,$(($atoms+1))p < $2 | cut -c 1-72)\n')
        out.write('*\n')
        out.write('EOF\n')
    else:
        out.write('cat > mol.in <<EOF\n')
        out.write('! {} tightscf noautostart $task cpcm BOHRS nopop\n'.format(level))
        out.write('%cpcm\nepsilon {}\nend\n'.format(eps))
        out.write('%pal nprocs {} end\n'.format(nprocs))
        out.write('%maxcore 8000\n')
        out.write('* xyz $charge $spin\n')
        ...
        out.write('*\n')
        out.write('EOF\n')
    out.write('echo \"running orca\"\n')
    out.write('/depot/bsavoie/apps/orca_5_0_1_openmpi411/orca mol.in > mol.out\n')
    out.write('echo \"Extracting data from ORCA outputs by extderi\"\n')
    out.write('../extorca $3 $atoms $derivs\n')
    out.write('mv mol.gbw tmp.gbw -f\n')
    out.write('rm mol.* mol_* -f\n')
    out.write('mv tmp.gbw mol.gbw -f\n')
    return
...
def predict_TS_intended(E,RG,PG,TS_output,model='IRC_model.json',package='Gaussian',Radj_mat=None,Padj_mat=None,lowTS_xyz=None,highTS_xyz=None,low_IRC=None):
    ...
    # determine bond changes and unchanged bonds
    Rbond_mat, _=find_lewis(E, Radj_mat)
    Pbond_mat, _=find_lewis(E, Padj_mat)
    BE_change   = Pbond_mat[0] - Rbond_mat[0]
    bond_break  = []
    bond_form   = []
    normal_bonds= []
    ...
    # load in lower level of theory TS
    if lowTS_xyz:
        _,low_G = xyz_parse(lowTS_xyz)
        _,DFT_G = xyz_parse(highTS_xyz)
    ...
    # use trained xgboost model to predict the intended rate proba
    try:
        intended_proba = IRCmodel.predict_proba(np.array([[Dis,bond_dis,RMSD,low_IRC,TS_Dis_mean,TS_Dis_min,TS_unexpect,dist]]))[0][1]
    except:
        intended_proba = IRCmodel.predict_proba(np.array([[TS_Dis_mean,TS_Dis_min,TS_unexpect,dist]]))[0][1]
