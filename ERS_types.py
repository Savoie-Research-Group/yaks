...
def generate_bond_form(bond_break,atom_list,lone=[],zw_ind=[],number_of_formation=2):
    ...
    elif len(lone)==2: # for biradical cases
        if number_of_formation==1: #b1f1
            if lone[0] not in atom_list and lone[1] not in atom_list:
                bond_form+=[[bond_break[0][0], lone[0]]]
                bond_form+=[[bond_break[0][1], lone[0]]]
                bond_form+=[[bond_break[0][0], lone[1]]]
                bond_form+=[[bond_break[0][0], lone[1]]]
                bond_form+=[[lone[0], lone[1]]]
            elif lone[0] in atom_list and lone[1] not in atom_list:
                bond_form+=[[bond_break[0][0], lone[1]]]
                bond_form+=[[bond_break[0][1], lone[1]]]
            elif lone[0] not in atom_list and lone[1] in atom_list:
                bond_form+=[[bond_break[0][0], lone[0]]]
                bond_form+=[[bond_break[0][1], lone[0]]]
        if number_of_formation==2: #b1f2
            if lone[0] not in atom_list and lone[1] not in atom_list:
                bond_form+=[[bond_break[0][0], lone[0]], [bond_break[0][1], lone[1]]]
                bond_form+=[[bond_break[0][0], lone[1]], [bond_break[0][1], lone[0]]]
    else:
...
def split_compounds(E,G,adj_mat, bond_mat=None, q_tot=0):
    # loop over all of the products and get inchikey for each component
    E=[_.lower() for _ in E]
    frag_inchi = []
    ...
    loop_ind = []
    if bond_mat is None:
        bond_mat, _ = find_lewis(E, adj_mat, q_tot)
        bond_mat=bond_mat[0]
    _, fc, _, _=count_bond_matrix(E, bond_mat)
    fc=fc[0]
    # check the charge to draw the correct smiles
    ...
    for group in groups:
        ...
        for count_i,i in enumerate(group):
            frag_G[count_i,:] = G[i,:]
            partial_charge=partial_charge+fc[i]
        # determine the adjacency matrix and write a mol file 
        frag_adj_mat= adj_mat[group,:][:,group]
        frag_B=bond_mat[group,:][:,group]
        inchikey = return_inchikey(frag_E,frag_G,frag_adj_mat, q_tot=partial_charge)
        smiles   = return_smi(frag_E,frag_G,adj_mat=frag_adj_mat, q_tot=partial_charge)
        inchi_list+= [inchikey]
        smile_list += [smiles]
    return smile_list,inchi_list,groups

# define type one reaction break 2 form 2 (b2f2) elementary reaction step
def R1_b2f2(reactant,truncate=[],ff='uff',phase=1,fixed=[]):
    ...
    lone=reactant['lone']
    #print(BE)
    if q_tot==0: radical_part=[count_i for count_i, i in enumerate(lone) if i%2!=0]
    else:
        radical_part=[]
        charge_atm=[i for i in fc if i!=0]
        tmp=BE                                                                                                                                                                                        
        if len(charge_atm)>1:
# Generate the coordinate covalent bond to see which atom should be reactive
            tmp=coordinate_covalent([i.lower() for i in E], BE)
        tmp_fc=return_formals(tmp, [i.lower() for i in E])
        e_tet = np.array([ find_lewis.n_electrons[_.lower()] for _ in E ])                                                                                                                        
        def_atom=return_def(tmp, [i.lower() for i in E], e_tet)
        for count, _ in enumerate(tmp_fc):
            if _<0 and BE[count, count]: # The charged atom with lone electrons = nucleophile
                radical_part.append(count)
            if _>0 and def_atom[count]<0:
                radical_part.append(count)  
    BE_list = []
    total_hash_list = []
    ...
    for bond_break in comb:
        ...
        if (bond_break[0][0] in freeze and bond_break[0][1] in freeze and BE[bond_break[0][0]][bond_break[0][1]]==1):
            continue

        if (bond_break[1][0] in freeze and bond_break[1][1] in freeze and BE[bond_break[1][0]][bond_break[1][1]]==1):
            continue

        # can't involve fixed atoms
        if bond_break[0][0] in fixed or bond_break[0][1] in fixed or bond_break[1][0] in fixed or bond_break[1][1] in fixed: continue
        ...
        if len(common_atom) == 0:
            ...
            for bond_form in bonds_form:
                ...
                check_flag = check_bond_condition(N_BE) and check_ring_condition(ring_index,N_BE)
                ...
                if 3 in truncate: check_flag = (check_flag and check_bridge(ring_index) and check_fused_condition(ring_index))
                ...
                if array_unique(N_BE_canon,BE_list)[0] and array_unique(N_hash_list,total_hash_list)[0] and check_flag:
                    ...
                    N_G = opt_geo(deepcopy(G),N_adj_mat,E,ff=ff,step=1000, q=q_tot)
                    Psmile_list,Pinchi_list,Pgroups = split_compounds(E,N_G,N_adj_mat, q_tot=q_tot)
                    ...
                    if array_unique(N_adj_mat,possible_product[N_p]["adj_list"])[0]:
                        ...
                        # Apply force field geometry optimization for each product and adj_mat
                        N_G = opt_geo(deepcopy(G),N_adj_mat,E,q=q_tot,ff=ff,step=1000)
                        possible_product[N_p]["G_list"]   += [deepcopy(N_G)]
                        possible_product[N_p]["adj_list"] += [deepcopy(N_adj_mat)] 
                        possible_product[N_p]["BE_list"]  += [deepcopy(N_BE)]
...
def R1_b3f3(reactant,truncate=[],ff='uff',phase=1,include_b2f2=True,limit_b3f3=False,fixed=[]):
...
# Create a list of potential products, BE_list and hash_list are used to identify whether a new prodcut is found or not
    ...
    # if phase is 2 and number of reactant is 2
        ...
        print("Phase 2 enumeration: bimolecular transformation...")
            ...
            if special_flag:
                ...
                for c_atom in connect_atoms:
                    ...
                    if array_unique(N_BE_canon,BE_list)[0] is True and array_unique(N_hash_list,total_hash_list)[0] is True and check_flag:
                        ...
                        # Apply force field geometry optimization for each product and adj_mat
                        N_G = opt_geo(deepcopy(G),N_adj_mat,E,ff=ff,q=q_tot,step=1000)
                        if len(N_G) != len(G):
                            N_G = N_G[:len(G)]
    ...
    for bond_break in comb3:
        ...
        if (bond_break[2][0] in freeze and bond_break[2][1] in freeze and BE[bond_break[2][0]][bond_break[2][1]]==1): 
            continue

        # can't involve fixed atoms
        if bond_break[0][0] in fixed or bond_break[0][1] in fixed or bond_break[1][0] in fixed or bond_break[1][1] in fixed or bond_break[2][0] in fixed or bond_break[2][1] in fixed: continue
...
def R2_zwitterionic(reactant,anion_ind,cation_ind,truncate=[],ff='uff',phase=1,fixed=[]):
    ...
    for bond_break in comb:
        ...
        if len(bond_break) == 1:
            ...
            if 3 in truncate:
                check_flag = (check_flag and check_bridge(ring_index) and check_fused_condition(ring_index))
...
def R2_b1f1(reactant,truncate=[], ff='uff',phase=1, fixed=[]):
    # create a dictionary for products and a dictionary for seperated uni-products
    possible_product = {}
    separated_product= {}
...
# Generate the coordinate covalent bond to see which atom should be reactive
    ...
    total_hash_list.append(deepcopy(hash_list))
    freeze   = return_freeze(E,adj_mat,BE)
    smile_list,inchi_list,groups = split_compounds(E,G,adj_mat, q_tot=q_tot)
    comb=[bc for bc in combinations(bond_list,1)]
    #else:
    #    comb=[]
    #    for bc in combinations(bond_list,1):
    #        for i in bc:
    #            for j in radical_part:
    #                if j in i: comb.append(bc)
    #print(comb)
    # initialzie some lists
    total_break = []
    bond_change_list = []
                ...
                for c_atom in connect_atoms:
                    ...
                    if array_unique(N_BE_canon,BE_list)[0] is True and array_unique(N_hash_list,total_hash_list)[0] is True and check_flag:
                        ...
                        if array_unique(N_adj_mat,possible_product[N_p]["adj_list"])[0]:
                            ...
                            possible_product[N_p]['type']+=['b1f1']
    return possible_product,separated_product
# break one form two rxns
def R2_b1f2(reactant, truncate=[], ff='uff', phase=1, fixed=[]):
    possible_product = {}
    separated_product= {}
...
# Create a list of potential products, BE_list and hash_list are used to identify whether a new prodcut is found or not
    ...
    for bond_break in comb:
        ...
        if len(common_atom) == 0:
            # if there is no common atom, two possible new arrangements
            BE_break  = break_bonds(BE,bond_break)
            bonds_form= generate_bond_form(bond_break,atom_list,lone=radical_part, number_of_formation=2)
            for bond_form in bonds_form:
                ...
                if array_unique(N_BE_canon,BE_list)[0] and array_unique(N_hash_list,total_hash_list)[0] and check_flag:
                    ...
                    possible_product[N_p]["adj_list"] =[deepcopy(N_adj_mat)]
                    possible_product[N_p]["BE_list"]  =[deepcopy(N_BE)]
                    possible_product[N_p]['type']=['b1f2']
                    # write info into separated product dictionary
                    for ind,group in enumerate(Pgroups):                           
                        inchi_ind = Pinchi_list[ind].split('-')[0]
...
# break two form one
def R2_b2f1(reactant, truncate=[], ff='uff', phase=1, start_ind=0):
...
# define type two (uni-radical) reaction break 2 form 2 (b2f2, include b1f1) elementary reaction step
# when include_b1f1 is true, enuemrate both b1f1 and b2f2 reactions
# other constraints: 1. radical-containing atom must involve in the reaction and 2. b2f2 must involve a single/double bond transform in both side  
def R2_b2f2(reactant,truncate=[],ff='uff',phase=1):
    # Use break 2 form 1 process to generate the radical products from neutral compounds
    possible_product = {}                             
    separated_product= {}
...
# Create a list of potential products, BE_list and hash_list are used to identify whether a new prodcut is found or not
    ...
    if phase == 1 and len(groups) == 1:
        print("Phase 1 enumeration: unimolecular transformation...")
        # generate all possible C_N^1 and C_N^2 combinations
        # comb1 = set([tuple(bond) for bond in bond_list if radical_atom not in bond])
        comb2 = [bc for bc in combinations(bond_list, 2)]
    # if phase is 2 and number of reactant is 2
    elif phase == 2 and len(groups) == 2:

        # find which molecule the radical locates
        if radical_atom in groups[0]: nonradical_mole = 1
        else: nonradical_mole = 0
        # generate all bond breaks in the close-shell molecule
        #comb1 = [bond for bond in set(bond_list) if bond[0] in groups[nonradical_mole]]
        # generate all possible C_N^2 combinations, only keep it when break two bonds in two reactant
        comb2 = [bc for bc in combinations(bond_list, 2) if [bc[0][0] in group for group in groups].index(True) != [bc[1][0] in group for group in groups].index(True) ]
        ...
        print("Only two phases available, make sure phase and number of reactant matches, quit...")
        quit()
    # set up constrained b2f2
    comb2 = [i for i in comb2 if (max([bond_list.count(bc) for bc in i]) > 1 and len(set([tuple(j) for j in i])) == 2)]
    # initialzie some lists
    total_break = []
    bond_change_list = []
    # loop over all b2 bond changes
    for bond_break in comb2:
        ...
        # can't break the bond involved the radical atom 
        if radical_atom in bond_break[0] or radical_atom in bond_break[1]: continue                                 
        ...
        if len(common_atom) == 0:
            ...
            # further select bond form based on limit
            bonds_form = [bf for bf in bonds_form if max([bond_list.count(b) for b in bf]) > 0]
