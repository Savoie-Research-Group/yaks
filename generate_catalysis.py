...
sys.path.append('/'.join(os.path.abspath(__file__).split('/')[:-1])+'/old_utilities')
# NOTE: find_lewis was updated and operates differently now.  This script calls the old utilities and therefore the old find_lewis
...
def main(argv):
    ...
    parser.add_argument('--exclude_b3f3', dest='excludeb3', default=False, action='store_const', const=True, help = 'When set, will not also generate water catalyzed pathways for b3f3 reaction')
...
def generate_catalysis(input_xyz,output_xyz,catalyst='H20',ff='uff',excludeH2=False,excludeb3=False):
    ...
    for input_file in input_xyzs:
        ...
        # print(bond_break, H2_form, H_index)
...
def return_reactive(E,RG,PG,return_Rmats=False):
    ...
    if len(bond_mat_2) > 0:
        # for Resonance structure, find the minimum bond change
        N_Bc = []
        for ind in range(len(bond_mat_2)):
            BE_2   = np.diag(lone_2[ind])+bond_mat_2[ind]
            BE_change = BE_2 - BE_1
            N_Bc.append(abs(BE_change).sum())

        # find the BE
        Nind = N_Bc.index(min(N_Bc))
        BE_2   = np.diag(lone_2[Nind])+bond_mat_2[Nind]
        BE_change = BE_2 - BE_1
