...
sys.path.append('/'.join(os.path.abspath(__file__).split('/')[:-3])+'/yaks/utilities')
...
def main(argv):
    ...
    parser = argparse.ArgumentParser(description='Driver script for further refinement based on previous YARP calculation results.')
...
def run_GSM(c):
    ...
    N_grad,reaction_dict,TSgjf_list = channel_classification(output_path=c["output_path"],initial_xyz=c['input_xyz'],gjf_list=TSgjf_list,xTB_IRC_dict=IRC_dict,N_dict=N_dict,adj_dict=adj_dict,F_dict=F_dict,reaction_dict=reaction_dict,\
                                                             TS_folder=TS_folder,append=True,model='{}//utilities/general_model.json'.format('/'.join(os.getcwd().split('/')[:-1])))
