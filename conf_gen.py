...
sys.path.append('/'.join(os.path.abspath(__file__).split('/')[:-4])+'/yaks/utilities')
# print('/'.join(os.path.abspath(__file__).split('/')[:-4])+'yaks/utilities')
...
def main(argv):
    ...
    if start not in [0,1,2]:
        print("Receiving invalid input! Sampling strategy should be 0, 1, or 2. Treat as 0...")
        start = 0
    ...
    for i in input_files:
        ...
        if start != 1 and R_index not in index_list: 
            ...
            print(Energy, opted_geo, finish)
            if not finish: continue
            # R-adj isn't pulling correctly! missing two bonds!
            # Check whether geo-opt changes the geometry. If so, geo-opt fails
            _,NRG = xyz_parse(reactant_file)
            print(R_adj-Table_generator(E,NRG))
            print(sum(sum(R_adj-Table_generator(E,NRG))))
            print(R_adj,"\n\n", Table_generator(E,NRG))
    ...
    print("filte folder",file_folder)
    print("conf_path", conf_path)
    print("opt_list", opt_list)
    print("finished", finished)
    print("path to job submit", '/'.join(os.getcwd().split('/')[:-1]))
    ...
    substring   = "python {}/utilities/job_submit.py -f 'CREST.*.submit' -sched {}".format('/'.join(os.getcwd().split('/')[:-2]),'slurm') # edited path to submit
    ...
    substring="python {}/utilities/job_submit.py -f 'CONF_GEN.*.submit' -sched {}".format('/'.join(os.getcwd().split('/')[:-2]),'slurm') # edited path to job.submit
    output = subprocess.Popen(substring,shell=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE).communicate()[0].decode('utf-8')
